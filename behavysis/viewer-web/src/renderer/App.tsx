...
import { Box, Grid, AppBar, Toolbar, Typography, Button, Container, CssBaseline, ThemeProvider, createTheme } from '@mui/material';
...
const App: React.FC = () => {
  ...
  const [keypointsModel, setKeypointsModel] = useState<KeypointsModel | null>(null);
  ...
  const handleOpenConfig = async () => {
    ...
    // Load Video
    const vidFp = await window.electronAPI.joinPath(rootDir, '2_formatted_vid', `${name}.mp4`);
    setVideoSrc(`file://${vidFp}`);

    // Load Keypoints (Parquet)
    const dlcFp = await window.electronAPI.joinPath(rootDir, '4_preprocessed', `${name}.parquet`);
    if (await window.electronAPI.pathExists(dlcFp)) {
        const dlcBuf = await window.electronAPI.readFile(dlcFp);
        const table = new Table(parquet.readParquet(new Uint8Array(dlcBuf)) as any);
    ...
    const bFp = await window.electronAPI.joinPath(rootDir, '7_scored_behavs', `${name}.parquet`);
    setBehavsFp(bFp);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const success = await window.electronAPI.writeFile(behavsFp.replace('.parquet', '.json'), uint8);
      if (success) {
  ...
  const updateBout = (updates: Partial<Bout>) => {
    if (boutsData && selectedBoutIndex >= 0) {
      ...
      newBouts[selectedBoutIndex] = { ...newBouts[selectedBoutIndex], ...updates };
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        <AppBar position="static" elevation={0}>
          <Toolbar variant="dense">
            <Typography variant="h6" sx={{ flexGrow: 1 }}>Behavysis Viewer</Typography>
            <Button color="inherit" onClick={handleOpenConfig}>Open</Button>
        ...
        <Box sx={{ flexGrow: 1, p: 2, overflow: 'hidden' }}>
          <Grid container spacing={2} sx={{ height: '100%' }}>
            ...
            <Grid item xs={12} md={4} sx={{ display: 'flex', flexDirection: 'column', gap: 2, height: '100%' }}>
              ...
              <Box sx={{ height: '40%' }}>
                <BoutInspector
                  bout={boutsData && selectedBoutIndex >= 0 ? boutsData.bouts[selectedBoutIndex] : null}
                  index={selectedBoutIndex}
                  onUpdate={updateBout}
                  onReplay={() => {
                      if (boutsData && selectedBoutIndex >= 0) {
                          setCurrentTime(boutsData.bouts[selectedBoutIndex].start / fps);
                      }
                  }}
                />
              </Box>
