...
class KeypointsModel:
    ...
    def __init__(self):
        self._load(KeypointsDf.init_df(pd.Series()), ExperimentConfigs())

    def _load(self, keypoints_df: pd.DataFrame, configs: ExperimentConfigs):
        """
        ...
        self.indivs_bpts_df = KeypointsAnnotationsDf.get_indivs_bpts(keypoints_df)
        self.colours = KeypointsAnnotationsDf.make_colours(keypoints_df, self.colour_level, self.cmap)

    def load(self, fp: str, configs: ExperimentConfigs):
        self._load(KeypointsDf.read(fp), configs)
