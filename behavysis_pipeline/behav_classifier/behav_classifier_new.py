...
class BehavClassifier:
    ...
    def create_from_project(cls, proj: Project) -> list[BehavClassifier]:
        ...
        # TODO: only using scored behaviour names from first experiment
        y_df = BehavClassifier.preprocess_y(
            BehavMixin.read_feather(
                proj.get_experiments()[0].get_fp(Folders.SCORED_BEHAVS.value)
            )
        )
    ...
    @classmethod
    def read_model(cls, configs_fp: str) -> GradientBoostingClassifier:
        """
    ...
    def preprocess_y(y: pd.DataFrame) -> pd.DataFrame:
        ...
        by expanding the `actual` and all specific outcome columns.
        """
        # Imputing NaN values with 0
        y = y.fillna(0)
    ...
    def prepare_data(self) -> tuple[np.ndarray, np.ndarray]:
        """
        Returns the X (samples, window, features) and y (samples, class) np arrays for the model.
        """
    ...
    def read_cnn_classifier(self):
        """
        __summary__
        """
        self.model = joblib.load(self.model_fp)

    def write_cnn_classifier(self):
        """
        __summary__
        """
        joblib.dump(self.model, self.model_fp)
    ...
    def init_model(self):
        ...
        # Input layers
        input_shape = (
            self.configs.window_frames * 2 + 1,
            546,
        )  # Numb of SimBA features
    ...
    def visualize_cnn_classifier(self):
        """
        __summary__
        """
    ...
    def train_cnn_classifier(self, x, y):
        """
        __summary__
        """
        h = self.model.fit(
    ...
    def evaluate_cnn_classifier(self, x, y):
        """
        __summary__
        """
        metrics_fig = BehavClassifier.metrics(self.model, x, y, pcutoff=0.4)
        metrics_fig.savefig(
            os.path.join(self.root_dir, f"{self.configs.behaviour_name}_metrics.png")
        )
        pcutoffs_fig = BehavClassifier.metrics_pcutoffs(self.model, x, y)
        pcutoffs_fig.savefig(
            os.path.join(self.root_dir, f"{self.configs.behaviour_name}_pcutoffs.png")
        )
    ...
    @staticmethod
    def metrics(model, x, y, pcutoff=0.5):
        y_pred = model.predict(x)
        y_pred = y_pred > pcutoff
    ...
    @staticmethod
    def metrics_pcutoffs(model, x, y):
        ...
        for i, pcutoff in enumerate(pcutoffs):
            y_pred_i = y_pred > pcutoff
            report = classification_report(y, y_pred_i, output_dict=True)
