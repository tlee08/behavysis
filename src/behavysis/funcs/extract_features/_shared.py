"""Shared constants, helpers, and infrastructure for HPW/rearing feature extraction.

Bodypart constants, kinematics helpers, rolling-window aggregation, floor
estimation, and likelihood-weighted bottom-reference computation.
"""
...
def _compute_bottom_reference(
    ...
    """
    n_frames = (
        keypoints_df.select("frame").unique().sort("frame").to_series().len()
    )
